...
class CategoryService {
  ...
  async validateCategoryData(categoryData, isUpdate = false, excludeId = null) {
    ...
    if (categoryData.name) {
      ...
      if (existingCategory) {
        errors.push(
          `Category name '${categoryData.name.trim()}' already exists. Please choose a different name.`
        );
      }
    }
    ...
    if (
      categoryData.meta_title !== undefined &&
      categoryData.meta_title !== null &&
      typeof categoryData.meta_title !== "string"
    ) {
  ...
  setupMulterConfiguration() {
    this.categoryImageUpload = multer({
      ...
      fileFilter: (req, file, cb) => {
        ...
        cb(
          new Error(
            `Category image upload only supports the following filetypes - ${filetypes}`
          )
        );
      },
    });
  ...
  async safeFileDelete(fileUrl, description = "file") {
    try {
      if (fileUrl && fileUrl.includes("digitaloceanspaces.com")) {
        ...
        this.logger.info(
          `Deleted ${description} from DigitalOcean Spaces: ${key}`
        );
      } else {
        this.logger.warn(
          `Invalid or missing ${description} URL for deletion: ${fileUrl}`
        );
      }
    } catch (err) {
      this.logger.error(
        `Error deleting ${description} from DigitalOcean Spaces ${fileUrl}: ${err.message}`
      );
    }
  }
  ...
  async createCategory(categoryData, imageFile) {
    try {
      ...
      const { name, description, parent_id, meta_title, sort_order, slug } =
        categoryData;
  ...
  async editCategory(categoryId, updateData) {
    try {
      ...
      // FIXED: Added slug to destructuring
      const {
        name,
        description,
        image_url,
      ...
      // REMOVED: Duplicate name validation - validateCategoryData already handles this
      // Only check name if it's being updated and is different from current name
      if (name && name.trim() !== category.name) {
        const existingCategory = await Category.findOne({
          where: {
            name: name.trim(),
      ...
      if (error.name === "SequelizeUniqueConstraintError") {
        throw {
          ...
          statusCode: 400,
          message:
            "A category with this name or slug already exists. Please choose different values.",
          details: [
            "Category names and slugs must be unique across the system.",
          ],
        };
      }
